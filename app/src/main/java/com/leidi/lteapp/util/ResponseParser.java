...
public class ResponseParser<T> extends TypeParser<T> {
    ...
    public T onParse(@NotNull okhttp3.Response response) throws IOException {
        ...
        if (data.getCode() == 401 && data.getMsg().contains("认证失败")) {
            EventBus.getDefault().post(new TokenInvalidEvent("token过期，请重新登录"));
        }
        if (data.getCode() != 200 && t == null) {//code不等于0，说明数据不正确，抛出异常
